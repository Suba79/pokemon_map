...
```bash
   git clone https://github.com/Suba79/pokemon_map.git
   cd pokemon_map
```
...
4. **Примените миграции (создаст файл базы данных):**
``` bash
...
- `SECRET_KEY` — секретный ключ проекта. В разработке можно использовать любое значение, например replace-me.
